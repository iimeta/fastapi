...
type Group struct {
	...
	IsEnableImageSizeCheck bool                  `bson:"is_enable_image_size_check,omitempty"` // 是否启用图像尺寸强校验
	ImageSizeCheckRetry    int                   `bson:"image_size_check_retry,omitempty"`     // 图像尺寸强校验重试次数, 0 表示跟随系统错误重试次数
