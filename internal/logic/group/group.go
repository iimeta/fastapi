...
func (s *sGroup) GetById(ctx context.Context, id string) (*model.Group, error) {
	...
	return &model.Group{
		...
		IsEnableImageSizeCheck: group.IsEnableImageSizeCheck,
		ImageSizeCheckRetry:    group.ImageSizeCheckRetry,
...
func (s *sGroup) List(ctx context.Context) ([]*model.Group, error) {
	...
	for _, result := range results {
		items = append(items, &model.Group{
			...
			IsEnableImageSizeCheck: result.IsEnableImageSizeCheck,
			ImageSizeCheckRetry:    result.ImageSizeCheckRetry,
...
func (s *sGroup) UpdateCache(ctx context.Context, newData *entity.Group) {
	...
	group := &model.Group{
		...
		IsEnableImageSizeCheck: newData.IsEnableImageSizeCheck,
		ImageSizeCheckRetry:    newData.ImageSizeCheckRetry,
