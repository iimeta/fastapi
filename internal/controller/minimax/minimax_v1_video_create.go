...
func (c *ControllerV1) VideoCreate(ctx context.Context, req *v1.VideoCreateReq) (res *v1.VideoCreateRes, err error) {
	...
	defer func() {
		logger.Debugf(ctx, "Controller MiniMax VideoCreate time: %d", gtime.TimestampMilli()-now)
	}()

	responseBytes, err := service.MiniMax().VideoCreate(ctx, g.RequestFromCtx(ctx), nil, nil)
	if err != nil {
		return nil, err
	}
